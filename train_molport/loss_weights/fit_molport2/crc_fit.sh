#!/bin/bash
#SBATCH --job-name=loss_weights_<JOB_NAME>
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
python3 <LIGAN_DIR>/generate.py --data_model ../<MODEL_DIR>/<DATA_MODEL_NAME>.model --gen_model ../<MODEL_DIR>/<GEN_MODEL_NAME>.model --gen_weights ../<WEIGHTS_DIR>/$TRAIN_JOB_NAME/${GEN_WEIGHTS_NAME}.caffemodel -r ${REC} -l ${LIG} --data_root <DATA_ROOT> -b lig -b lig_gen --fit_atoms --output_sdf --output_channels --verbose 3 --gpu <GEN_OPTIONS> --n_samples <N_SAMPLES> --beam_size <BEAM_SIZE> --threshold <THRESHOLD> --peak_value <PEAK_VALUE> --min_dist <MIN_DIST> --learning_rate <LEARNING_RATE> --beta1 <BETA1> --beta2 <BETA2> --weight_decay <WEIGHT_DECAY> --interm_gd_iters <INTERM_GD_ITERS> --final_gd_iters <FINAL_GD_ITERS> -o <JOB_NAME>_${SLURM_ARRAY_TASK_ID}
